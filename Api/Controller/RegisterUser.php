...
class RegisterUser
{
  ...
  public static function setNewTeacher($request)
  {
    ...
    $obTeacher->classMatter   = $postVars['classMatter'];
  ...
  /**
   * Método responsável por cadastrar um novo admistrador
   */
  public static function setNewAdmin($request)
  {
    ...
    $obAdmin = AdminRepository::findAdmin('', $where);

    // VERIFICA SE O ADMIN JÁ EXISTE
    if ($obAdmin) {
      return [
        "Error" => true,
        "message" => "Já existe um administrador cadastrado com esse número siape!"
      ];
    }
    ...
    if ($obAdmin[0]->email === $postVars['email']) {
      return [
        "Error" => true,
        "message" => "Email já utilizado por outro administrador!"
      ];
    }
